...
void buildMakefile(void)
{
    ...
    if (parms.video)
    {
        ...
        strcat(objs, "    $(O)/d_display.c  \\\n");
        strcat(objs, "    $(O)/v_video.c    \\\n");
        strcat(objs, "    $(O)/v_def.c      \\\n");
        strcat(objs, "    $(O)/v_memory.c   \\\n");
        strcat(objs, "    $(O)/r_render.c   \\\n");
        strcat(objs, "    $(O)/r_commands.c \\\n");
        strcat(objs, "    $(O)/r_pipeline.c \\\n");
    }
    if (parms.audio)
    {
        strcat(libs, "-lportaudio ");
        strcat(deps, "    a_audio.h    \\\n");
        strcat(objs, "    $(O)/a_audio.c \\\n");
    }
    if (parms.input)
    {
        strcat(deps, "    i_input.h    \\\n");
        strcat(objs, "    $(O)/i_input.c \\\n");
    }
    strcat(libs, "\n");
    strcat(deps,     "    m_math.h     \\\n");
    strcat(deps,     "    utils.h      \\\n");
    strcat(deps,     "    def.h        \\\n");
    strcat(objs,     "    $(O)/m_math.c     \\\n");
    strcat(objs,     "    $(O)/utils.c      \\\n");
